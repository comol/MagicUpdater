...
namespace ForceStaticUpdate1CPlugin
{
	public class ForceStaticUpdate1CPlugin : OperationWithAttr<PluginAttrib>
	{
		public ForceStaticUpdate1CPlugin(int? operationId, string attrsJson) : base(operationId, attrsJson)
		{
		}
	...
	{
		public string Description => $"Обновление с завершением сеансов пользователей"; //Описание операции - будет отображаться в подсказке пользователей

		public int GroupId => 1;  // группа операции - чем меньше тем приоритетнее

		public string NameRus => "Обновление с завершением сеансов пользователей"; //Название операции как оно будет отображаться пользователю
	}
