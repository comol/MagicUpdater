...
namespace FileUpdate1CPlugin
{
	public class FileUpdate1CPlugin : OperationWithAttr<PluginAttrib>
	{
		public FileUpdate1CPlugin(int? operationId, string attrsJson) : base(operationId, attrsJson)
		{
		}
	...
	{
		public string Description => $"Обновление (файловая база)"; //Описание операции - будет отображаться в подсказке пользователей

		public int GroupId => 1;  // группа операции - чем меньше тем приоритетнее

		public string NameRus => "Обновление (файловая база)"; //Название операции как оно будет отображаться пользователю
	}
