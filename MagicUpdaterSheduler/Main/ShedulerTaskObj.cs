...
namespace MagicUpdaterSheduler.Main
{
	public class ShedulerTaskObj : ShedulerTask
	{
		...
		//private ShedulerStepObj CheckStepResultNew(ShedulerStepObj step, int computerId)
		//{
		//	MLogger.Debug($"\"CheckStepResultNew\", ThreadId: {Thread.CurrentThread.ManagedThreadId}, OperationId: {step.OperationID}");
		//	var operation = MQueryCommand.SelectOperationById(step.OperationID);
		...
		//		if (!(operation.IsRead ?? false))
		//		{
		//			var typeName = ((OperationsType)operation.OperationType).ToString();
		//			MQueryCommand.TryInsertShedulerHistory(Id, step.Id, false, $"Операция \"{typeName}\" для компьютера \"{operation.ComputerName}\" не прочитана. ID операции = {operation.ID}");
		//			return null;
		//		}
		...
		//		if ((operation.IsRead ?? false) && (!(operation.IsCompleted ?? false) || (operation.Is1CError == 1)))
		//		{
		//			while (step.RepeatCount > 0)
		//			{
		//				step.RepeatCount -= 1;
		//				Thread.Sleep(step.RepeatTimeout ?? 0);
		//				step.Run(computerId);
		//				var localOperation = MQueryCommand.SelectOperationById(step.OperationID);
		...
		//						continue;
		//					}
		//					else if ((operation.IsRead ?? false) && (operation.IsCompleted ?? false) && (operation.Is1CError == 0))
		//					{
		//						return GetStep(step.OnOperationCompleteStep);
		//					}
		//					else if (!(operation.IsRead ?? false))
		//					{
		//						var typeName = ((OperationsType)operation.OperationType).ToString();
		//						MQueryCommand.TryInsertShedulerHistory(Id, step.Id, false, $"Операция \"{typeName}\" для компьютера \"{operation.ComputerName}\" не прочитана. ID операции = {operation.ID}");
		//						return null;
		...
		//					{
		//						var typeName = ((OperationsType)operation.OperationType).ToString();
		//						MQueryCommand.TryInsertShedulerHistory(Id, step.Id, false, $"Не удалось проверить состояние операции \"{typeName}\" для компьютера \"{operation.ComputerName}\". ID операции = {operation.ID}");
		//						return null;
		//					}
		//				}
		//				else
		//				{
		//					MQueryCommand.TryInsertShedulerHistory(Id, step.Id, false, $"Ошибка получения операции");
		//					return null;
		//				}
		//			}

		//			return GetStep(step.OnOperationErrorStep);
		//		}
